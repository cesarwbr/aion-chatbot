...
module.exports = function() {
  ...
  this.insert = function(topic, data) {
    ...
    _.map(data, function(item) {
      classifier.addDocument(item.pattern.toUpperCase(), topic);
    });
    classifier.train();
  };
  ...
  this.bestMatch = function(input) {
    ...
    _.map(dataTopic, function(item) {
      var gradient = jscosine.textCosineSimilarity(item.pattern.toUpperCase(), input);
    ...
    var answerSentiment = defaultValue(humourMap[inputSentiment],'neutral');

    var best = gradientMap[bestGradient];

    return defaultValue(best.answer[answerSentiment],best.answer['neutral']);
  ...
  function defaultValue(myVar, defaultVal){
    if( myVar === undefined) myVar = defaultVal;
    return myVar;
  }
